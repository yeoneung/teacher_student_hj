"""Same budget/acceptance implementation as frozen v8a; only teacher model changes.

The training function is copied verbatim from study.py with these dependencies
replaced: CurvatureGraph, collect_curvature, curvature_teaching_data and
curvature_predicted_reduction. This keeps the archived v8a source unchanged.
"""
import copy
...
from .targets import RegressionGraph
from .curvature import CurvatureGraph
from .curvature_training import collect_curvature,curvature_teaching_data,curvature_predicted_reduction

def train_curvature(p,cfg,seed,spec,data,out):
    ...
    jet=CurvatureGraph(teacher,p,cfg['batch'])
    ...
    while elapsed()<maximum:
        start=time.perf_counter()
        prediction=None if cache is None else curvature_predicted_reduction(actor,labels,cache,p)
        teacher.load_state_dict(actor.state_dict())
        costs,new_cache=collect_curvature(jet,initial,p,cfg['batch']);torch.cuda.synchronize()
        ...
        if cache is not None:
            labels,metrics=curvature_teaching_data(actor,cache,p,regularization,spec['kind'])
