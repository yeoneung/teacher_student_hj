...
from impact_learning import data_at,labels,student_states,audit_operation
    ...
    while elapsed()<budget:
        ...
        if method=='guided':
            ...
            if operation=='refresh':
                xx,tt=student_states(student,p,trainx[:32],2048,gen)
                fresh=data_at(teacher,p,xx,tt,jet,0.,'none',seed+blocks*43)
                current={k:(torch.cat([v[:2048],fresh[k]]) if isinstance(v,torch.Tensor) else v) for k,v in data.items()}
...
def run(stage):
    ...
    cfg=dict(stage=stage,seeds=seeds,methods=methods,selected_fixed=selected,budget=15.,
    ...
    for fi,family in enumerate(['mechanical','reaction']):
        ...
        for si,seed in enumerate(seeds):
            ...
            for original_method in order:
                ...
                student,result=train(p,teacher,parent,data,jet,value,trainx,valx,method,seed,common_seconds,15.)
...
if __name__=='__main__':
    args=argparse.ArgumentParser();args.add_argument('--stage',choices=['pilot','replication_v2'],required=True)
